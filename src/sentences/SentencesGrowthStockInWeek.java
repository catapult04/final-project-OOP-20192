...
public class SentencesGrowthStockInWeek extends SentencesGrowthStock {
    ...
    private static final String[] PERCENT_CHANGE = {
	        " có giá so với tuần trước bằng ",
            " có sự thay đổi về giá so với tuần trước là : ",
            " với giá là so với tuần trước bằng "
	};

	private static final String TOTAL_VOLUME = "có khối lượng giao dịch trong tuần là: ";
	...
	private static final String[] MAX_PRICE = {
	        " có giá cao nhất trong tuần là ",
            " là mã có giá cao nhất trong tuần : ",
	...
	private static final String[] MAX_TOTAL_VOLUME = {
            ...
            " dù được giao dịch nhiều nhất trong tuần với khối lượng là ",
            " dù đang rất được quan tâm với khối lượng giao dịch nhiều nhất "
	};

	private static final String[] MIN_TOTAL_VOLUME = {
	        " và đang có khối lượng mua bán trong tuần nằm ở vị trí cuối bảng ",
            " đang chững lại với khối lượng giao dịch thấp nhất tuần- ",
            " nhưng là mã được giao dịch thấp nhất tuần với "
	};

	private static final String[] MAX_PERCENT_CHANGE = {
	        " có sự thay đổi về giá cao nhất so với tuần trước, tăng: ",
            " tuần này đã có sự tăng giá nhiều nhất so với các mã còn lại: ",
            " tăng giá rất mạnh so với tuần qua: "
	};

	private static final String[] MIN_PERCENT_CHANGE = {
	        " giảm giá vô cùng mạnh, so với tuần trước giảm đến: ",
            " có sự tụt giảm về giá lớn nhất trong tuần: ",
            " đang là mã cố phiếu giảm giá nhiều nhất: "
	};

	private static final String[] MAX_AVG_TRADE = {
	        " có khối lượng giao dịch trung bình trong một tuần tương đối cao, đứng ở vị trí đầu bảng: ",
            " là mã cổ phiếu có khối lượng giao dịch lớn nhất tuần qua, đạt mức: ",
            " trong tuần lã mã có khối lượng giao dịch lớn nhất, có số liệu là: "
	};

	private static final String[] MIN_AVG_TRADE = {
	        " có khối lượng giao dịch trung bình trong một tuần tương đối thấp, đứng ở vị trí cuối bảng: ",
            " là mã cổ phiếu có khối lượng giao dịch thấp nhất tuần qua, đạt mức: ",
            " trong tuần lã mã có khối lượng giao dịch thấp nhất, có số liệu là: "
	};
    ...
    return "Như vậy, thống kê biến động giá có một số đặc biệt : "
            + "\n Trong tuần này, " + SentencesGrowthStockInWeek.generateEvaluationOfMinPrice(list)
            + "\n Đối lập lại, " + SentencesGrowthStockInWeek.generateEvaluationOfMaxPrice(list)
            + "\n Trong khi đó," + SentencesGrowthStockInWeek.generateEvaluationOfMinTotalVolume(list)
            + "\n Ngược lại, " + SentencesGrowthStockInWeek.generateEvaluationOfMaxTotalVolume(list)
            + "\n Tiếp theo, " + SentencesGrowthStockInWeek.generateEvaluationOfMaxAvgTrage(list)
            + "\n Bên phía ngược lại, " + SentencesGrowthStockInWeek.generateEvaluationOfMinAvgTrage(list)
            + "\n Cũng không bất ngờ khi " + SentencesGrowthStockInWeek.generateEvaluationOfMaxPercentChange(list)
            + "\n Bên cạnh đó " + SentencesGrowthStockInWeek.generateEvaluationOfMinPercentChange(list)
            + "\n Cuối cùng, " + SentencesGrowthStockInWeek.generateEvaluationOfMaxTotalAsset(list)
            + "\n Và " + SentencesGrowthStockInWeek.generateEvaluationOfMinTotalAsset(list)
