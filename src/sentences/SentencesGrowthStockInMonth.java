...
public class SentencesGrowthStockInMonth {
    ...
    private static final String[] PERCENT_CHANGE = {
            " có giá so với tháng trước bằng ",
            " có sự thay đổi về giá so với tháng trước là : ",
            " với giá là so với tháng trước bằng "
    };

    private static final String TOTAL_VOLUME = "có khối lượng giao dịch trong tháng là: ";
    ...
    private static final String[] MAX_PRICE = {
            " có giá cao nhất trong tháng là ",
            " là mã có giá cao nhất trong tháng : ",
    ...
    private static final String[] MAX_TOTAL_VOLUME = {
            ...
            " dù được giao dịch nhiều nhất trong tháng với khối lượng là ",
            " dù đang rất được quan tâm với khối lượng giao dịch nhiều nhất "
    };

    private static final String[] MIN_TOTAL_VOLUME = {
            " và đang có khối lượng mua bán trong tháng nằm ở vị trí cuối bảng ",
            " đang chững lại với khối lượng giao dịch thấp nhất tháng- ",
            " nhưng là mã được giao dịch thấp nhất tháng với "
    };

    private static final String[] MAX_PERCENT_CHANGE = {
            " có sự thay đổi về giá cao nhất so với tháng trước, tăng: ",
            " tháng này đã có sự tăng giá nhiều nhất so với các mã còn lại: ",
            " tăng giá rất mạnh so với tháng qua: "
    };

    private static final String[] MIN_PERCENT_CHANGE = {
            " giảm giá vô cùng mạnh, so với tháng trước giảm đến: ",
            " có sự tụt giảm về giá lớn nhất trong tháng: ",
            " đang là mã cố phiếu giảm giá nhiều nhất: "
    };

    private static final String[] MAX_AVG_TRADE = {
            " có khối lượng giao dịch trung bình trong một tháng tương đối cao, đứng ở vị trí đầu bảng: ",
            " là mã cổ phiếu có khối lượng giao dịch lớn nhất tháng qua, đạt mức: ",
            " trong tháng là mã có khối lượng giao dịch lớn nhất, có số liệu là: "
    };

    private static final String[] MIN_AVG_TRADE = {
            " có khối lượng giao dịch trung bình trong một tháng tương đối thấp, đứng ở vị trí cuối bảng: ",
            " là mã cổ phiếu có khối lượng giao dịch thấp nhất tháng qua, đạt mức: ",
            " trong tháng là mã có khối lượng giao dịch thấp nhất, có số liệu là: "
    };
    ...
    public static String generateSentences(List <GrowthStock> list) {
        ...
        return "Như vậy, thống kê biến động giá có một số đặc biệt : "
                + "\n Trong tháng này, " + SentencesGrowthStockInMonth.generateEvaluationOfMinPrice(list)
                + "\n Đối lập lại, " + SentencesGrowthStockInMonth.generateEvaluationOfMaxPrice(list)
                + "\n Trong khi đó," + SentencesGrowthStockInMonth.generateEvaluationOfMinTotalVolume(list)
                + "\n Ngược lại, " + SentencesGrowthStockInMonth.generateEvaluationOfMaxTotalVolume(list)
                + "\n Tiếp theo, " + SentencesGrowthStockInMonth.generateEvaluationOfMaxAvgTrage(list)
                + "\n Bên phía ngược lại, " + SentencesGrowthStockInMonth.generateEvaluationOfMinAvgTrage(list)
                + "\n Cũng không bất ngờ khi " + SentencesGrowthStockInMonth.generateEvaluationOfMaxPercentChange(list)
                + "\n Bên cạnh đó " + SentencesGrowthStockInMonth.generateEvaluationOfMinPercentChange(list)
                + "\n Cuối cùng, " + SentencesGrowthStockInMonth.generateEvaluationOfMaxTotalAsset(list)
                + "\n Và " + SentencesGrowthStockInMonth.generateEvaluationOfMinTotalAsset(list)
                + "\n Để biết thêm thông tin chi tiết, vui lòng truy cập: https://s.cafef.vn/thong-ke/timeline-1-thang/san-all.chn";
    }
}
